...
def run_extraction_stage(config):
    ...
    for source in sources:
        for brand in brands:
            try:
                ...
                # Получити статус екстракту з БД (Extractor оновлює статуси)
                try:
                    status = extractor.get_extract_status()
                except Exception as e:
                    logger.error(f"Could not read extract status from DB for {brand['name']} @ {source['name']}: {e}")
                    status = None
                ...
                if status == 'success':
                    ...
                    extraction_results.append({
                        ...
                        'brand': brand['name'],
                        'status': 'failed',
                        'error': f'extract_status={status}'
                    })
                
            except Exception as e:
                logger.error(f"Extraction failed for {brand['name']} from {source['name']}: {e}")
                extraction_results.append({
                    'source': source['name'],
                    'brand': brand['name'],
                    'status': 'failed',
    ...
    for result in extraction_results:
        ...
        logger.info(f"  {status} {result['source']} - {result['brand']}: {result['status']}")
